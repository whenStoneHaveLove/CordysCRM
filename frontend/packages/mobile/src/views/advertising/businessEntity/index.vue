<template>
  <div class="flex h-full flex-col overflow-hidden bg-[var(--text-n9)]">
    <van-nav-bar :title="t('advertising.businessEntity')" left-arrow @click-left="$router.back()">
      <template #right>
        <CrmIcon name="iconicon_filter" width="20px" height="20px" @click="filterVisible = true" />
        <van-icon
          v-if="hasPermission('AD_BUSINESS_ENTITY:CREATE')"
          name="plus"
          class="ml-[14px]"
          @click="onCreate"
        />
    ...
    <CrmList
      ref="listRef"
      class="flex-1"
      :load-list-api="getAdBusinessEntityPage"
      :keyword="keyword"
      ...
      <template #item="{ item }">
        <div class="mb-[12px] rounded-lg bg-white px-[12px] py-[10px]" @click="onItemClick(item)">
          ...
          <div class="mt-[4px] one-line-text text-[12px] text-[var(--text-n3)]">
            {{
              [item.code, item.isCrossEntity ? t('advertising.crossEntity') : '', item.remark]
                .filter(Boolean)
                .join(' · ') || '-'
            }}
          </div>
          <div class="mt-[2px] text-[12px] text-[var(--text-n3)]">
            {{ t('advertising.entityStat', { user: item.userCount ?? 0, order: item.orderCount ?? 0 }) }}
          </div>
        </div>
      </template>
    </CrmList>
...
<script setup lang="ts">
  ...
  import { getAdBusinessEntityPage } from '@/api/modules';
  ...
  const filterFields = [
    { key: 'status', label: t('advertising.field.status'), options: AdBusinessEntityStatusOptions },
  ];
  ...
  function onCreate() {
    router.push({ name: AdvertisingRouteEnum.AD_BUSINESS_ENTITY_FORM });
  }
</script>
