<template>
  <div class="flex h-full flex-col overflow-hidden bg-[var(--text-n9)]">
    <van-nav-bar :title="t('advertising.upstreamAgent')" left-arrow @click-left="$router.back()">
      <template #right>
        <CrmIcon name="iconicon_filter" width="20px" height="20px" @click="filterVisible = true" />
        <van-icon
          v-if="hasPermission('AD_UPSTREAM_AGENT:CREATE')"
          name="plus"
          class="ml-[14px]"
          @click="onCreate"
        />
    ...
    <CrmList
      ref="listRef"
      class="flex-1"
      :load-list-api="getAdUpstreamAgentPage"
      :keyword="keyword"
      ...
      <template #item="{ item }">
        <div class="mb-[12px] rounded-lg bg-white px-[12px] py-[10px]" @click="onItemClick(item)">
          ...
          <div class="mt-[4px] one-line-text text-[12px] text-[var(--text-n3)]">
            {{ [item.signingEntity, item.businessEntityName].filter(Boolean).join(' · ') || '-' }}
          </div>
        </div>
      </template>
    </CrmList>
...
<script setup lang="ts">
  ...
  import { getAdUpstreamAgentPage } from '@/api/modules';
  ...
  function onCreate() {
    router.push({ name: AdvertisingRouteEnum.AD_UPSTREAM_AGENT_FORM });
  }
