<template>
  <div class="flex h-full flex-col overflow-hidden bg-[var(--text-n9)]">
    <van-nav-bar :title="t('advertising.seal')" left-arrow @click-left="$router.back()">
      <template #right>
        <CrmIcon name="iconicon_filter" width="20px" height="20px" @click="filterVisible = true" />
        <van-icon
          v-if="hasPermission('AD_SEAL:APPLY')"
          name="plus"
          class="ml-[14px]"
          @click="onCreate"
        />
      ...
      <template #item="{ item }">
        <div class="mb-[12px] rounded-lg bg-white px-[12px] py-[10px]" @click="onItemClick(item)">
          ...
          <div class="mt-[4px] one-line-text text-[12px] text-[var(--text-n3)]">
            {{
              [item.contractNo, item.businessEntityName, item.sealTypeLabel].filter(Boolean).join(' · ') || '-'
            }}
          </div>
          <div class="mt-[2px] text-[12px] text-[var(--text-n3)]">
            {{ t('advertising.sealCopies', { applied: item.appliedCopies ?? 0, actual: item.actualCopies ?? 0 }) }}
          </div>
        </div>
      </template>
    </CrmList>
...
<script setup lang="ts">
  ...
  import { AdSealRecordStatusOptions } from '@lib/shared/enums/advertisingEnum';
  ...
  function onCreate() {
    router.push({ name: AdvertisingRouteEnum.AD_SEAL_APPLY });
  }
  ...
  const filterFields = [
    { key: 'status', label: t('advertising.field.status'), options: AdSealRecordStatusOptions },
  ];
