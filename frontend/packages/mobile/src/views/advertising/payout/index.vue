<template>
  <div class="flex h-full flex-col overflow-hidden bg-[var(--text-n9)]">
    <van-nav-bar :title="t('advertising.payout')" left-arrow @click-left="$router.back()">
      <template #right>
        ...
        <div class="mb-[12px] rounded-lg bg-white px-[12px] py-[10px]" @click="onItemClick(item)">
          <div class="flex items-center justify-between gap-[8px]">
            <span class="one-line-text text-[14px] font-medium text-[var(--text-n1)]">
              {{ item.paymentNo || item.orderName || '-' }}
            </span>
            <CrmTag v-if="item.statusLabel" :tag="item.statusLabel" />
          </div>
          <div class="mt-[4px] one-line-text text-[12px] text-[var(--text-n3)]">
            {{ [item.orderName, item.typeLabel, item.mediaNames].filter(Boolean).join(' · ') || '-' }}
          </div>
          <div class="mt-[2px]">
            <CrmTag :tag="getAdPayoutBillTypeLabel(item.billType)" />
          </div>
...
<script setup lang="ts">
  ...
  import { getAdPayoutPage } from '@/api/modules';
  ...
  function onCreate() {
    router.push({ name: AdvertisingRouteEnum.AD_PAYOUT_FORM });
  }
  ...
  const listParams = computed(() => ({
    ...
    billType: searchForm.value.billType ?? null,
  }));
  ...
  const filterFields = [
    { key: 'status', label: t('advertising.field.status'), options: AdPayoutStatusOptions },
    { key: 'type', label: t('advertising.field.paymentType'), options: AdPayoutTypeOptions },
    { key: 'billType', label: t('advertising.field.billType'), options: AdPayoutBillTypeOptions },
  ];
