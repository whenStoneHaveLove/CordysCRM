 ...
 * 响应由框架 {@code ResultResponseBodyAdvice} 统一包裹。</p>
 *
 * <p>生命周期：申请(apply) → 审批通过(approve)/驳回(reject) → 执行上传(upload)。
 * 鉴权由 {@link AdSealRecordService} 依据角色做守卫（同 M2/M3 风格），未接入审批模块（M6 收口）。</p>
 */
@Tag(name = "广告用印")
@RestController
