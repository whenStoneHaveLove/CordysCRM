 ...
 *
 * <p>接口级权限由 {@link CsPermission} 依据 {@link PermissionConstants} 的 AD_CUSTOMER_* 码校验。</p>
 */
@Tag(name = "广告客户")
@RestController
...
public class AdCustomerController {
    ...
    @PostMapping
    @CsPermission(PermissionConstants.AD_CUSTOMER_CREATE)
    ...
    @PutMapping
    @CsPermission(PermissionConstants.AD_CUSTOMER_UPDATE)
    ...
    @GetMapping("/{id}")
    @CsPermission(PermissionConstants.AD_CUSTOMER_READ)
    ...
    @DeleteMapping("/{id}")
    @CsPermission(PermissionConstants.AD_CUSTOMER_DELETE)
    @Operation(summary = "删除广告客户")
    public void delete(@PathVariable("id") String id) {
        adCustomerService.delete(id, userId(), orgId());
    }
}
