...
(
    `id`                 VARCHAR(32)  NOT NULL COMMENT 'id',
    `resource_type`      TINYINT      NOT NULL COMMENT '资源类型:10代理/20',
    `name`               VARCHAR(255) NOT NULL COMMENT '名称',
    `media_type`         VARCHAR(32)  COMMENT '类型(字典)',
...
(
    ...
    `media_payable_amount`      DECIMAL(15,2) NOT NULL COMMENT '应付总额(下游口径,L-28)',
    ...
    `receipt_method`            TINYINT      NOT NULL COMMENT '收款方式:10预收/20账期',
    ...
    `payment_method`            TINYINT      NOT NULL COMMENT '付款方式:10预付/20后付',
    `payment_prepay_mode`       TINYINT      COMMENT '预付模式:10比例/20固定',
    `payment_prepay_ratio`      DECIMAL(5,2) COMMENT '预付比例%',
    `payment_prepay_amount`     DECIMAL(15,2) COMMENT '预付金额(基数=media_payable,L-28)',
    `payment_prepay_deadline`   DATE         COMMENT '预付截止日',
    ...
    `media_payment_status`      TINYINT      NOT NULL DEFAULT 0 COMMENT '付款进度:0未付/10部分/20全额',
    ...
    `media_paid_amount`         DECIMAL(15,2) NOT NULL DEFAULT 0 COMMENT '已付款累计',
...
(
    ...
    `related_party_type` TINYINT      NOT NULL COMMENT '10客户/20上游代理/30下游客户',
...
(
    ...
    `business_entity_id` VARCHAR(32) NOT NULL COMMENT '业务主体',
    `resource_id`        VARCHAR(32) COMMENT '关联/代理(L-23)',
    `direction`          TINYINT     NOT NULL COMMENT '10上游收款/20下游付款',
    `type`               TINYINT     NOT NULL COMMENT '10预收/20预付/30开票收款/40尾款/50退款/60坏账',
